    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (msg.includes("parse") || msg.includes("PDF") || msg.includes("text")) {
          throw new Error("Cannot process resume.");
        }
        throw new Error(msg || "Failed to match resume.");
      }
      ...
      setError(error.message || "Server is offline or could not process resume.");
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleUploadFromModal = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        const msg = data?.error ?? "";
        if (msg.includes("parse") || msg.includes("PDF") || msg.includes("text")) {
          throw new Error("Could not process the resume.");
        }
        throw new Error(msg || "Failed to match resume.");
      }
      const matches = (data.matches ?? data).filter((j: Job) => j.similarity >= 60);
